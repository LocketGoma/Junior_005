...
#include "stdafx.h"
//중위 중위
#include <iostream>				//lv2 태그 : 제 2형 TOY
using namespace std;
InfixToPostfix::InfixToPostfix(CString Infix_tmp){
	...
	Infix->Replace("MOD", " MOD ");			//lv2
	Infix->Replace("FACT", "FACT ");		//lv2
...
//후위표기식으로 만드는 함수
void InfixToPostfix::make_Postfix(){//-0--- 이거 바꿔
	int i = 0;
...
int InfixToPostfix::Is_open_parentheses(int index){  //---------------------
	...
	for (i = index + 1; i < len; i++){
		...
		//lv2 팩토리얼 전용. 팩토리얼 전용 제거시 요거만 빼면 됨
		else if (isalpha(tmpbuf[i])) {
			i = Is_op(i);
			if (errnum) {		//lv2 여기서 에러는 에러가 아니므로 패스. 
				continue;
			}
	...
	for (; i < len; i++){
		...
		else if (this_op_num == 3) {		//lv2, FACT일때 패스.	FACT 전용 제거시 요거만 제거.
			break;							//브레이크 안해주면 여기서 계속 돌아요
		}
		else if (isalpha(tmpbuf[i])){ // 기호니?  <------------ 기호 넣기
			i = Is_op(i);
			this_op_num = current_opnum;
			if (errnum){ // 안들어간다고?
				return i;
...
//			tmp_str.AppendChar(tmpbuf[i]);
	...
	//		return i;
		...
		//잘못된 기호 들어옴
			...
			if (isdigit(tmpbuf[i]))			//여기?
				errnum = 4;					//lv2 FACT 쓰면 여기서 물림.
			else
				errnum = 3;
			return i;
		}
	...
	//term 안나오고 문장 끝
	if (i == len){				//lv2 이젠 여기서 걸리네
		errnum = 2;
		return i - 1;
	}
	tmp_str.AppendChar(' ');//------------------------------- tmp_str : 변환된 문자열 / AppendChar() <- 공백 제어?
	//seq 2: <term>
	for (; i < len; i++){
